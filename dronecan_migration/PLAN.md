...
> **▶ Resume point — next session starts at P4.3.** P4.0–P4.2 are done and committed on `dronecan-libcanard` (scaffold `376dc0c8df`, transport `f6621d27fd`, P4.2 record `a7c70410cd`). The libcanard transport/codec/router stack exists and is contract-tested (`src/drivers/dronecan/test/run_codec_test.sh` — keep it green), and the Q1 flatness thesis is confirmed. **P4.3 = wire `DronecanNode` to the transport:** port `CanardNuttXCDev`/`CanardSocketCAN` from cyphal re-typed to the v0 `CanardCANFrame` (DESIGN §2/§8); construct-inject one into a `DronecanHandle` member; replace the P4.0 `canardInit` link-proof in `DronecanNode::print_info()` with real handle ownership; lazy first-`Run()` bring-up + CAN-RX-IRQ `ScheduleNow()`; `transmit(); receive(); transmit();` under `_node_mutex` (DESIGN §6); `canardSetLocalNodeID(DC_NODE_ID)` after `isUnicast` validation; NodeStatus @1 Hz; `dronecan status` to surface node id + `peak_usage_blocks`; `migrateLegacyParams()` via the out-of-module hook + rcS keyed on `DC_ENABLE` (DESIGN §7). Acceptance: node boots on `ark_fmu-v6x`, appears on the bus, `status` shows the node id. Full detail in the dated P4.0/P4.1/P4.2 entries below; architecture in [`DESIGN.md`](DESIGN.md) §8.
